...
int GetMARMUX(int *x)        { return((x[MARMUX1] << 1) + x[MARMUX0]); } //lab 5
...
int GetLD_JNEXT(int *x)      { return(x[LD_JNEXT]); }
int GetGATE_PTE(int *x)      { return(x[GATE_PTE]); }
int GetMBIT(int *x)          { return(x[MBIT]); }
...
void eval_micro_sequencer() {
   ...
   */
    ...
    if(GetLD_JNEXT(uinst)){ //get the state translation needs to return to
        switch (CURRENT_LATCHES.STATE_NUMBER){
            case 18: NEXT_LATCHES.JNEXT = 33; break;
            case 19: NEXT_LATCHES.JNEXT = 33; break;
            case 15: NEXT_LATCHES.JNEXT = 28; break;
            case 2: NEXT_LATCHES.JNEXT = 29; break;
            case 6: NEXT_LATCHES.JNEXT = 25; break;
            case 7: NEXT_LATCHES.JNEXT = 23; break;
            case 3: NEXT_LATCHES.JNEXT = 24; break;
        }
    }

    //PRIORITY LOGIC 
    int opcode = (CURRENT_LATCHES.IR >> 12) & 0xF;    
    if(CURRENT_LATCHES.STATE_NUMBER == 52){//CHECK UNALIGNED EXCEPTION
        int wordinst = 0;
        if(CURRENT_LATCHES.JNEXT == 33 || opcode == 6 || opcode == 7 || opcode == 15) //fetch
            wordinst = 1;
        if(wordinst && (CURRENT_LATCHES.MAR & 0x1)==1){
            NEXT_LATCHES.EXCV = 0x03; //unaligned vector
            EXC_SIGNAL = 1;
        }
    }else if(CURRENT_LATCHES.STATE_NUMBER == 55){//CHECK PROTECTION/PAGEFAULT
        int p = (CURRENT_LATCHES.MDR >> 3) & 0x1;
        int v = (CURRENT_LATCHES.MDR >> 2) & 0x1;
        if(CURRENT_LATCHES.PRIVILEGE && (p==0)){ //protection
            NEXT_LATCHES.EXCV = 0x04;
            EXC_SIGNAL = 1;
        }else if(v==0){ // page fault
            NEXT_LATCHES.EXCV = 0x02;
            EXC_SIGNAL = 1;
        }
    }else if(CURRENT_LATCHES.STATE_NUMBER == 32){ //UNKNWON OPCODE
    ...
    if(ird == 1){
        ...
        }else if (cond == 5){  // [STORE]
            int store = (opcode == 7 || opcode == 3);
            state = j | (store << 4);
        }
    }
    //priority mux
    if(EXC_SIGNAL){ //exception
        state = 10;
    //interrupt
    }else if(INT_SIGNAL && (CURRENT_LATCHES.STATE_NUMBER == 18 || CURRENT_LATCHES.STATE_NUMBER == 19)){
        state = 36;
        NEXT_LATCHES.INTV = 0x01;
        INT_SIGNAL = 0;
    //Jnext
    }else if(GetTRANS(uinst)){
        state = CURRENT_LATCHES.JNEXT;
    }
...
void eval_bus_drivers() {
   ...
   */    
    ...
    int* uinst = CONTROL_STORE[CURRENT_LATCHES.STATE_NUMBER];
    // GATE PTE ----------------------
    int newpte = CURRENT_LATCHES.MDR | 0x1; // set reference
    if(GetMBIT(uinst))
        newpte |= 0x2; //  set modified
    g_pte = newpte;
    ...
    if(GetMARMUX(uinst)==0){ //00: 7.0 select LSHF(ZEXT[IR[7.0]],1)  TRAP
        g_marmux = (CURRENT_LATCHES.IR & 0xFF) << 1;
    }else if(GetMARMUX(uinst)==1){ // 01: addr
        int addr1 = 0;
        int addr2 = 0;
        // ADDR1
    ...
    }else if(GetMARMUX(uinst)== 2){ // physical address = MDR[13:9] + VA[8:0]
        g_marmux = (CURRENT_LATCHES.MDR & 0x3E00) | (CURRENT_LATCHES.VA & 0x01FF);
    }else if(GetMARMUX(uinst)==3){ //PTE = PTBR[15:8] + LSHF(VA[15:9], 1)
        g_marmux = (CURRENT_LATCHES.PTBR & 0xFF00) | (((CURRENT_LATCHES.VA >> 9) & 0x007F) << 1);
    }
