...
void MEM_stage() {
  ...
  if(v_dcache_en && Get_DCACHE_RW(PS.MEM_CS)){
    //store
  ...
  int addr = PS.MEM_ADDRESS;
  targetPC = PS.MEM_ADDRESS;
  //mem alu result logic block, like mdr you wanna store byte/word
  int cachedata;
  if(Get_DATA_SIZE(PS.MEM_CS)){
    cachedata = PS.MEM_ALU_RESULT;
  }else{
    if(PS.MEM_ADDRESS & 0x1){
      cachedata = ((PS.MEM_ALU_RESULT & 0xFF) << 8) & 0xFF00;
    }else{
      cachedata = PS.MEM_ALU_RESULT & 0xFF;
    }
  }
  ...
  int dcache_r = 1;
  if(v_dcache_en)
    dcache_access(PS.MEM_ADDRESS, &read_word, cachedata, &dcache_r, WE0, WE1);

  if(v_dcache_en && (dcache_r==0))
    mem_stall = 1;
  else
    mem_stall = 0;

  //data logic block
  trapPC = read_word; // dcache returns address from trap vector table = trapPC
  int srdata;
  if(Get_DATA_SIZE(PS.MEM_CS)){ //word
    srdata = Low16bits(read_word);
  }else{ //byte
    if(PS.MEM_ADDRESS & 0x1){ //odd byte
      srdata = sext(((read_word >> 8) & 0xFF), 8);
    }else{
      srdata = sext((read_word & 0xFF), 8);
    }
  }
  ...
  if(PS.MEM_V ){
    ...
    if(br_op && (PS.MEM_CC & ((PS.MEM_IR >> 9)& 0x7))){ //does nzp share 1 with cc
      mem_pcmux = 1; //target
    }else if(uncon_op){
      mem_pcmux = 1; //target
    }else if(trap_op){
  ...
  // see if older instructions arent bubbles and writing to a register or updatinf cc
  ...
  if(mem_stall)
    ...
    NEW_PS.SR_V = PS.MEM_V; //propagate exisitng bubble or not
...
void AGEX_stage() {
  ...
  if(addrmux){
    ...
    int addr1out;
    if (addr1mux)
      addr1out = PS.AGEX_SR1;
    else
      addr1out = PS.AGEX_NPC;

    int addr2mux = Get_ADDR2MUX(PS.AGEX_CS);
    if(addr2mux == 1){
      addr2out = sext((PS.AGEX_IR & 0x3F), 6); // offset6
    }else if(addr2mux == 2){
      addr2out = sext((PS.AGEX_IR & 0x1FF), 9); //offset9
    }else if(addr2mux == 3){
      addr2out = sext((PS.AGEX_IR & 0x7FF), 11); //offest11
    }
    if (Get_LSHF1(PS.AGEX_CS))
      addr2out = addr2out << 1;

    addrmuxout = addr1out + addr2out;
  }else{
    addrmuxout = (PS.AGEX_IR & 0xFF) << 1;
  }
  addrmuxout = Low16bits(addrmuxout);
  ...
  if(Get_ALU_RESULTMUX(PS.AGEX_CS)){
    ...
    if(sr2mux){
      sr2out = sext(PS.AGEX_IR & 0x1F, 5);
    }else{
      sr2out = PS.AGEX_SR2;
    }
    if(Get_ALUK(PS.AGEX_CS)==0) //ADD
        aluresult = PS.AGEX_SR1 + sr2out;
    else if(Get_ALUK(PS.AGEX_CS)==1) //AND
        aluresult = PS.AGEX_SR1 & sr2out;
    else if(Get_ALUK(PS.AGEX_CS)==2) //XOR
        aluresult = PS.AGEX_SR1 ^ sr2out;
    else if(Get_ALUK(PS.AGEX_CS)==3) //PASS B
        aluresult = sr2out;
  }else{
    //shift
    int steer = (PS.AGEX_IR >> 4) & 0x3;
    int amount4 = PS.AGEX_IR & 0xF;
    if(steer == 0 || steer == 2){  //LSHF
        aluresult = PS.AGEX_SR1 << amount4;
    }else if(steer == 1){ //RSHFL
        aluresult = (PS.AGEX_SR1 & 0xFFFF) >> amount4;
    }else if(steer == 3){ //RSHFA
        aluresult = sext(PS.AGEX_SR1 & 0xFFFF, 16) >> amount4;
    }  
  }
...
void DE_stage() {
  ...
  int CONTROL_STORE_ADDRESS = ((PS.DE_IR >> 10) & 0x3E) | ((PS.DE_IR >> 5) & 0x1);  /* You need to implement the logic to
  ...
  int* constore = CONTROL_STORE[CONTROL_STORE_ADDRESS];
  int sr1 = (PS.DE_IR >> 6) & 0x7;
  int sr2;
  if ((PS.DE_IR >> 13) & 0x1) 
    sr2 = (PS.DE_IR >> 9) & 0x7;  // ir[11:9]
  else
    sr2 = PS.DE_IR & 0x7;  // ir[2:0]

  
  int nzp = (N<<2) | (Z<<1) | P; //latched previous nzp
  int sr1val = REGS[sr1];
  int sr2val = REGS[sr2];

  //load cc logic 
  if (v_sr_ld_reg) { //write to reg file when storing result
        REGS[sr_reg_id] = sr_reg_data;
    }
    
  if (v_sr_ld_cc) {//update global nzp when valid storing result
      N = sr_n;
      Z = sr_z;
      P = sr_p;
  }
  ...
  int dr = 7;
  if (Get_DRMUX(constore)==0)
    dr = (PS.DE_IR >> 9) & 0x7;
  // dependency stall :
  // get all control signal inputs for current instruction
  int sr1_needed = Get_SR1_NEEDED(constore);
  int sr2_needed = Get_SR2_NEEDED(constore);
  int de_br_op = Get_DE_BR_OP(constore);

  // stall if older instruction writing to the sr1 or sr2 thats needed
  int stallSR = (sr1_needed && (((PS.AGEX_DRID == sr1) && v_agex_ld_reg) || ((PS.SR_DRID == sr1) && v_sr_ld_reg) || ((PS.MEM_DRID ==sr1) && v_mem_ld_reg)))
  || (sr2_needed && (((PS.AGEX_DRID == sr2) && v_agex_ld_reg) || ((PS.SR_DRID == sr2) && v_sr_ld_reg) || ((PS.MEM_DRID ==sr2) && v_mem_ld_reg)));
  int stallCC = de_br_op && (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc);
  
  if((stallCC || stallSR) && PS.DE_V)
    dep_stall = 1;
  else 
    dep_stall = 0;
  
  // v.de.br.stall
  if(Get_DE_BR_STALL(constore) && PS.DE_V)
    v_de_br_stall = 1;
  else
    v_de_br_stall = 0;
  ...
  if (LD_AGEX) {
    ...
    if(dep_stall)
      NEW_PS.AGEX_V = 0; //bubble
    else
      NEW_PS.AGEX_V = PS.DE_V;  // carry through what might be a bubble
...
void FETCH_stage() {
  ...
  int pcstall = mem_stall || dep_stall || v_de_br_stall || v_agex_br_stall || v_mem_br_stall || !icache_r;
  int ld_pc = !pcstall || (mem_pcmux != 0 && !mem_stall); //no pc stall or branching somewhere in mem stage
  
  int ld_de = !(mem_stall || dep_stall); // no memory or dependency stall

  
  if(ld_pc){  // pcmux
    if(mem_pcmux == 0){
      PC = Low16bits(curPC + 2);
    }else if(mem_pcmux == 1){
      PC = Low16bits(targetPC);
    }else if(mem_pcmux == 2){
      PC = Low16bits(trapPC);
    }
  }
    
  if(ld_de) { //ld.de logic
    NEW_PS.DE_NPC = Low16bits(curPC + 2);  //new pc
    NEW_PS.DE_IR = instr; // icache access instruction
    if (v_de_br_stall || v_agex_br_stall || v_mem_br_stall || !icache_r) // not valid if stalled or icache not ready
      NEW_PS.DE_V = 0; 
    else
      NEW_PS.DE_V = 1;
    }
